...
public class SpaceDefectTester extends SpaceCompletenessTester {
	 ...
	 * @param workerId
	 */
	public SpaceDefectTester(final String workerId) {
		super(workerId);
	}
	...
	protected void check(final Computer computer) {
		// start transaction
		final TransactionReference tx;
		try {
			tx = capi.createTransaction(5000, space);
		} catch ( MzsCoreException e ) {
			log.error("Tester: %s could not test PC %s because it could not start a transaction.", workerId, computer.id );
			e.printStackTrace();
			return;
		}
		...
		try {
			...
			try {
				...
				log.info("Tester: %s could not rollback transaction.", workerId);
				e1.printStackTrace();
			}
		}
	}
